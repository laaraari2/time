...
function Assignments({ project }: { project: SavedScheduleProfile }) {
  const teacherCards = project.teachers.map((teacher) => {
    const assignments = (teacher.weeklyHoursAssignments ?? []).map((assignment) => { const classGroupId = assignment.classGroupId || assignment.classId || ''; const weeklyHours = Number(assignment.weeklyHours ?? assignment.hours ?? 0) || 0; const classGroup = project.classes.find((item) => item.id === classGroupId); const subject = project.subjects.find((item) => item.id === assignment.subjectId); return { assignment, classGroup, subject, weeklyHours }; }).filter((item) => item.classGroup && item.weeklyHours > 0);
  ...
  return <section><div className="mb-3 rounded-2xl bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-black">إسناد الأقسام</h2><p className="mt-1 text-[11px] text-slate-500">كل أستاذ والأقسام المسندة إليه وعدد الساعات الأسبوعية.</p></div><div className="shrink-0 rounded-xl bg-emerald-50 px-3 py-2 text-center"><b className="block text-lg font-black text-emerald-700">{totalHours}</b><span className="text-[9px] font-bold text-emerald-700">ساعة/أسبوع</span></div></div><div className="mt-3 grid grid-cols-2 gap-2 text-center text-[10px] font-bold"><div className="rounded-xl bg-slate-50 p-2">الأساتذة المسندون <b className="text-[#20518D]">{assignedTeachers.length}</b></div><div className="rounded-xl bg-slate-50 p-2">مجموع الأساتذة <b className="text-[#20518D]">{project.teachers.length}</b></div></div></div>{assignedTeachers.length > 0 && <div className="space-y-3">{assignedTeachers.map(({ teacher, assignments, totalHours: teacherTotal }) => <article key={teacher.id} className="overflow-hidden rounded-2xl bg-white shadow-sm"><div className="flex items-center justify-between gap-3 border-b bg-[#123E70] p-3 text-white"><div className="min-w-0"><b className="block truncate">{teacher.name}</b>{teacher.code ? <span className="text-[9px] text-white/70">{teacher.code}</span> : null}</div><div className="shrink-0 rounded-xl bg-white/15 px-3 py-1.5 text-center"><b className="block text-base">{teacherTotal}</b><span className="text-[8px] text-white/75">ساعات</span></div></div><div className="divide-y">{assignments.map(({ assignment, classGroup, subject, weeklyHours }, index) => <div key={`${teacher.id}-${assignment.classGroupId || assignment.classId || index}`} className="flex items-center justify-between gap-2 p-3"><div className="min-w-0"><b className="block text-sm">{classGroup?.code ?? classGroup?.name ?? 'قسم'}</b><span className="text-[10px] text-slate-500">{subject?.name ?? 'مادة غير محددة'}</span></div><span className="shrink-0 rounded-lg bg-blue-50 px-2 py-1 text-[10px] font-black text-[#20518D]">{weeklyHours} س</span></div>)}</div></article>)}</div>}{unassignedTeachers.length > 0 && <div className="mt-3 rounded-2xl bg-white p-4 shadow-sm"><h3 className="font-black">أساتذة بدون إسناد</h3><div className="mt-2 flex flex-wrap gap-2">{unassignedTeachers.map(({ teacher }) => <span key={teacher.id} className="rounded-full bg-amber-50 px-3 py-1 text-[10px] font-bold text-amber-700">{teacher.name}</span>)}</div></div>}</section>;
}
...
function Structure({ project }: { project: SavedScheduleProfile }) {
  ...
  const structureByClass = project.classes.map((classGroup) => {
    const rows = project.teachers.flatMap((teacher) => (teacher.weeklyHoursAssignments ?? []).map((assignment) => { const assignedClassId = assignment.classGroupId || assignment.classId || ''; if (assignedClassId !== classGroup.id) return null; const hours = Number(assignment.weeklyHours ?? assignment.hours ?? 0) || 0; if (hours <= 0) return null; const subject = project.subjects.find((item) => item.id === assignment.subjectId); return { id: `${teacher.id}-${assignment.subjectId}-${assignedClassId}`, subjectName: subject?.name ?? 'مادة غير محددة', hours }; }).filter((item): item is { id: string; subjectName: string; hours: number } => Boolean(item)));
