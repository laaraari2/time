...
export default function TeacherMobilePage() {
  ...
  async function load() {
    ...
    try {
      ...
      setProject(first as TeacherProject | null);
    } catch (err) {
      console.error(err);
    ...
    return project.placements
      .map((placement) => {
        ...
        return {
          ...
          day: days[placement.dayIndex] ?? fallbackDays[placement.dayIndex] ?? '',
      ...
      .sort((a, b) => a.dayIndex - b.dayIndex || a.periodIndex - b.periodIndex);
  }, [project, days]);

  const todayRows = rows.filter((row) => row.dayIndex === day);

  if (loading) {
    return <main dir="rtl" className="min-h-screen grid place-items-center bg-slate-100 font-bold">جاري تحميل جدولك...</main>;
  }
    ...
    <main dir="rtl" className="min-h-screen bg-slate-100 pb-8">
      <header className="sticky top-0 z-40 bg-[#123E70] px-4 pb-4 pt-[max(12px,env(safe-area-inset-top))] text-white shadow-lg">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] text-white/60">TimeTables — فضاء الأستاذ</p>
            <h1 className="truncate text-xl font-black">جدولي</h1>
          </div>
        ...
        {project && <p className="mt-2 text-xs text-white/75">{project.name}</p>}
      </header>
      ...
      <div className="mx-auto max-w-xl p-3">
        {error ? (
          ...
          <div className="rounded-2xl bg-white p-7 text-center font-bold text-slate-500">لا يوجد جدول مربوط بهذا الحساب.</div>
        ) : (
          <>
            <div className="mb-3 rounded-2xl bg-white p-3 shadow-sm">
              <div className="flex gap-2 overflow-x-auto pb-1">
                {days.map((name, index) => (
                  <button key={`${name}-${index}`} type="button" onClick={() => setDay(index)} className={`shrink-0 rounded-xl px-4 py-2 text-xs font-black ${day === index ? 'bg-[#20518D] text-white' : 'bg-slate-100 text-slate-600'}`}>{name}</button>
                ))}
              </div>
            </div>

            <div className="mb-3 flex items-center justify-between">
              <div><h2 className="text-xl font-black">{days[day]}</h2><p className="text-[10px] text-slate-500">جدول الحصص الخاص بك فقط</p></div>
              <span className="rounded-full bg-blue-50 px-3 py-1 text-[10px] font-black text-[#20518D]">{todayRows.length} حصص</span>
            </div>

            {todayRows.length === 0 ? (
              <div className="rounded-2xl bg-white p-8 text-center text-sm font-bold text-slate-500">لا توجد حصص مبرمجة لك هذا اليوم.</div>
            ) : (
              <div className="space-y-2">
                {todayRows.map((row) => (
                  <article key={row.id} className="rounded-2xl bg-white p-3 shadow-sm">
                    <div className="flex items-center gap-3">
                      <div className="w-20 shrink-0 rounded-xl bg-blue-50 p-2 text-center text-[#20518D]">
                        <Clock3 className="mx-auto h-4 w-4" />
                        <b className="mt-1 block text-[10px]">{row.start}</b>
                        <small className="text-[9px] text-slate-400">{row.end}</small>
                      </div>
                      <div className="min-w-0 flex-1">
                        <b className="block text-base">{row.subject}</b>
                        <span className="text-[10px] text-slate-500">{row.className}</span>
                      </div>
                    </div>
                  </article>
                ))}
            ...
            <div className="mt-4 rounded-2xl bg-white p-4 shadow-sm">
              <div className="flex items-center gap-2 text-[#20518D]"><CalendarDays className="h-5 w-5" /><b>الأسبوع</b></div>
              <p className="mt-1 text-[10px] text-slate-500">مجموع الحصص المبرمجة لك: <b>{rows.length}</b></p>
            </div>
          </>
        )}
      </div>
