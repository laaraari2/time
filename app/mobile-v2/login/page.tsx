...
export default function MobileLoginPage() {
  ...
  if (checking) return <main dir="rtl" className="min-h-screen grid place-items-center bg-slate-100 font-bold">جاري التحقق من الجلسة...</main>;
    ...
    <main dir="rtl" className="min-h-screen bg-gradient-to-br from-[#123E70] via-[#20518D] to-[#2B68B1] p-4 grid place-items-center">
      <div className="w-full max-w-sm rounded-[28px] bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-3">
          <div><label htmlFor="mobile-email" className="mb-1.5 block text-xs font-black">حساب الدخول</label><div className="relative"><Mail className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input id="mobile-email" type="text" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="username" placeholder="PROF001 أو PROF001@prof.com" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pr-10 pl-3 text-sm outline-none focus:border-[#2B68B1] focus:bg-white" /></div></div>
