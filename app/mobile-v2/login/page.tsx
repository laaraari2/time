...
export default function MobileLoginPage() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      if (data.session) window.location.replace('/mobile-v2');
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.replace('/mobile-v2');
    } catch (err) {
      console.error(err);
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-gradient-to-br from-[#123E70] via-[#20518D] to-[#2B68B1] p-4 grid place-items-center">
      <div className="w-full max-w-sm rounded-[28px] bg-white p-6 shadow-2xl">
        ...
        <div className="mt-4 text-center"><h1 className="text-2xl font-black">TimeTables Mobile</h1><p className="mt-2 text-xs leading-5 text-slate-500">سجّل الدخول للوصول إلى جداولك وإسناداتك من الهاتف.</p></div>
        <form onSubmit={handleSubmit} className="mt-6 space-y-3">
          <div><label htmlFor="mobile-email" className="mb-1.5 block text-xs font-black">البريد الإلكتروني</label><div className="relative"><Mail className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input id="mobile-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="username" placeholder="name@example.com" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pr-10 pl-3 text-sm outline-none focus:border-[#2B68B1] focus:bg-white" /></div></div>
        ...
        <p className="mt-5 text-center text-[9px] font-bold text-slate-400">نفس الحساب والبيانات الموجودة في نسخة الكمبيوتر</p>
      </div>
    </main>
  );
}
