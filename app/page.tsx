...
async function getTimetableRole(): Promise<'manager' | 'teacher' | 'none'> {
  try {
    ...
    if (data?.role === 'manager' || data?.role === 'teacher' || data?.role === 'none') return data.role;
  } catch (error) {
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!loginEmail.includes('@')) {
        loginEmail = normalizeTeacherLoginEmail(loginEmail);
        loginPassword = getTeacherAuthPassword(password);
      }
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-blue-50 px-4 py-6 sm:px-6 lg:py-10">
        ...
        <div className="w-full max-w-5xl overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-slate-200 lg:h-[470px] lg:flex">
          <section className="relative overflow-hidden bg-gradient-to-br from-[#123E70] via-[#20518D] to-[#2B68B1] p-6 text-white sm:p-7 lg:w-1/2 lg:p-8">
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-black backdrop-blur-sm"><Calendar className="h-4 w-4 text-amber-300" />TimeTables</div>
                <h1 className="mt-6 text-2xl font-black leading-tight sm:text-3xl">نظّم وقتك،<br /><span className="text-amber-300">وابنِ جدولك بثقة.</span></h1>
                <p className="mt-3 max-w-md text-[13px] leading-6 text-white/75 sm:text-sm">منصة واحدة لإعداد استعمال الزمن، تنظيم الأساتذة والأقسام والقاعات، ثم إنشاء جدول واضح وسهل التسيير.</p>
              </div>
              <div className="mt-5 flex items-center justify-center lg:mt-0"><div className="relative w-48 rotate-[-2deg] rounded-2xl border border-white/20 bg-white/10 p-3 shadow-xl backdrop-blur-sm sm:w-60"><div className="flex h-6 items-center gap-1 rounded-t-lg bg-white/15 px-3"><span className="h-1.5 w-1.5 rounded-full bg-red-300" /><span className="h-1.5 w-1.5 rounded-full bg-amber-300" /><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /></div><div className="mt-3 grid grid-cols-5 gap-1.5">{Array.from({ length: 15 }).map((_, index) => <div key={index} className={`h-3.5 rounded ${index % 5 === 1 ? 'bg-amber-300/80' : index % 4 === 0 ? 'bg-white/40' : 'bg-white/15'}`} />)}</div><div className="absolute -bottom-4 -right-4 flex h-10 w-10 rotate-[8deg] items-center justify-center rounded-xl bg-amber-400 text-slate-900 shadow-lg"><Sparkles className="h-5 w-5" /></div></div></div>
